...
The UI uses the REST API, including for relationship selections and all writes, and is localized in English and Japanese (browser locale by default, with a sidebar override). Device, VM, and Prefix details show related records. No free IP rows are generated for display. `/docs` serves a Swagger UI explorer backed by the generated OpenAPI document; its assets are served same-origin and the page relaxes only the style/image/font directives Swagger needs. Security headers disallow framing and restrict scripts and connections to the same origin. Browser mutations reject cross-origin requests; non-browser clients need no Origin header.
